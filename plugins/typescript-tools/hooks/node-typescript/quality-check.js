...
class TypeScriptConfigCache {
	...
	findTsConfigFiles() {
		...
		if (globSync) {
			try {
				...
				// Glob failed, fall through to manual search
			}
		}
	...
	loadCache() {
		try {
			const cacheContent = fsSync.readFileSync(this.cacheFile, 'utf8');
	...
	saveCache() {
		try {
			...
			fsSync.writeFileSync(this.cacheFile, JSON.stringify(this.cache, null, 2));
		} catch (e) {
	...
	getTsConfigForFile(filePath) {
		...
		if (relativePath.includes('src/webview/') || relativePath.includes('/webview/')) {
			...
			if (fsSync.existsSync(webviewConfig)) {
		...
		if (
			...
			if (fsSync.existsSync(testConfig)) {
				return testConfig;
			}
		}
...
function loadConfig() {
	...
	try {
		...
		if (fsSync.existsSync(configPath)) {
			fileConfig = JSON.parse(fsSync.readFileSync(configPath, 'utf8'));
		}
	} catch (e) {
...
let ESLint, prettier, ts, globSync;
...
async function loadOptionalDependencies() {
	...
	try {
		...
		log.debug('glob not found in project - will use fallback config search');
	}
}
...
class QualityChecker {
	...
	async checkCommonIssues() {
		...
		try {
			...
			// Check for type assertions (matching @typescript-eslint/consistent-type-assertions: never)
			const typeAssertionRule = config._fileConfig.rules?.typeAssertions || {};
			if (
				(this.fileType === 'typescript' || this.fileType === 'component') &&
				typeAssertionRule.enabled !== false
			) {
				lines.forEach((line, index) => {
					// Match 'as any', 'as Type', but not 'as const' (which is allowed as a literal type)
					// Also check for angle bracket assertions like <Type>value
					const hasAsAssertion = / as (?!const\b)\w+/.test(line);
					const hasAngleBracketAssertion = /<\w+>/.test(line) && !line.includes('<svg') && !line.includes('React.FC<') && !line.includes('Promise<');
					
					if (hasAsAssertion || hasAngleBracketAssertion) {
						const severity = typeAssertionRule.severity || 'error';
						const message =
							typeAssertionRule.message || 'Type assertions are not allowed. Use proper typing instead.';

						if (severity === 'error') {
							this.errors.push(`Found type assertion in ${this.filePath} - ${message}`);
							console.error(`  Line ${index + 1}: ${line.trim()}`);
							foundIssues = true;
						} else {
							// Warning level - just warn, don't block
							log.warning(`Type assertion at line ${index + 1}: ${message}`);
						}
					}
			});
			}
 ...
 */
async function main() {
	// Load optional dependencies first
