...
function displayResults(
	...
	if (errors.length > 0) {
		...
		for (const error of errors) {
			console.error(
				`  ${colors.red}❌${colors.reset} ${colors.yellow}Line ${error.line}:${error.column}${colors.reset} ` +
	...
	if (config.showWarnings && warnings.length > 0) {
		...
		for (const warning of warnings) {
			console.error(
				`  ${colors.yellow}⚠️${colors.reset} ${colors.yellow}Line ${warning.line}:${warning.column}${colors.reset} ` +
...
async function main(): Promise<void> {
	...
	if (!isTypeScriptFile(filePath)) {
		...
		console.error(`${colors.yellow}⏭️  Skipped - not a TypeScript file${colors.reset}`);
		console.error('');
		process.exit(0);
	}
	...
	try {
		...
		if (errors.length > 0) {
			if (config.blockOnErrors) {
				...
				console.error(`${colors.yellow}⚠️  TypeScript errors found but not blocking${colors.reset}`);
